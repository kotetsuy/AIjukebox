...
For English, see [TECHNICAL.md](TECHNICAL.md).
...
VOICEVOX の既定音量は音楽に対して小さい(実測ピーク 0.34FS / RMS -22.6dBFS)。
`[voicevox] volume_scale` を `audio_query` の `volumeScale` に流し込んで上げる。

| volumeScale | ピーク | RMS |
|---|---|---|
| 1.0 | 0.34FS | -22.6dBFS |
| 2.0 | 0.68FS | -16.6dBFS |
| 2.2(採用) | 0.74FS | -15.8dBFS |
| 2.5 | 0.84FS | -14.7dBFS |
| 3.0 | 1.00FS | -13.1dBFS(クリップ) |

**ピークは発話ごとに 1.4 倍ほど振れる。** 上表は基準文のもので、実測した
コメントの素のピークは 0.34〜0.47FS に散らばっていた。つまり最も大きい発話に
合わせるなら 2.0 が上限で、2.2 ではそういう発話が数サンプル(最長 0.125ms)
天井に当たる。その長さでは可聴域に出ないので許容している。

ダッキングの `p` を下げて相対的に大きくする手も取れるが、それだと音楽ごと
体感音量が下がるだけなので、声そのものを上げる。

**キャッシュ済みの wav は古い音量のまま**なので、変えたら作り直すこと。
`.json` に text が残っているので、Qwen を呼ばずに再合成できる。
